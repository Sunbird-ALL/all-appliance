...
.controller('QuestionCreationFormController', ['$scope', 'instance', 'questionData', function ($scope, instance, questionData) {
  ...
  $scope.setPreviewData = function () {
    ...
    questions.push(qObj);
    this.myQuestion = $scope.questionCreationFormData;
  ...
  $scope.sendMetaData = function (newQuestionCreate) {
    ...
    $scope.saveMetaData($scope.assessmentId, $scope.myQuestion);
  ...
  $scope.saveQuestion = function (assessmentId, data) {
    $scope.savingQuestion = true;
  	ecEditor.getService('assessment').saveQuestionV3(assessmentId, data, function (err, resp) {
  		// if (!err) {
        $scope.savingQuestion = false;
  			var qMetadata = data.request.assessment_item.metadata;
...
org.ekstep.pluginframework.pluginManager.registerPlugin({"id":"org.ekstep.question","ver":"1.0","author":"Jagadish","title":"Question Plugin","description":"Plugin to create questions","editor":{"main":"editor/plugin.js","dependencies":[{"type":"css","src":"editor/style.css"},{"type":"js","src":"editor/question-ctrl.js"},{"type":"plugin","plugin":"org.ekstep.questionset.preview","ver":"1.1"},{"type":"plugin","plugin":"org.ekstep.sunbirdmetadata","ver":"1.1"}],"menu":[],"init-data":{},"configManifest":[{}],"help":{"src":"editor/help.md","dataType":"text"}},"renderer":{"main":"renderer/plugin.js","dependencies":[{"type":"plugin","plugin":"org.ekstep.questionset.preview","ver":"1.1"}]},"icon":"assets/icon.png","languages":["English"],"categories":[""],"keywords":[""]},org.ekstep.question={},org.ekstep.question.EditorPlugin=org.ekstep.contenteditor.basePlugin.extend({type:"org.ekstep.question",initialize:function(){var e=this,t=(ecEditor.addEventListener("org.ekstep.question:showpopup",this.loadHtml,this),ecEditor.resolvePluginResource(e.manifest.id,e.manifest.ver,"editor/question.html")),e=ecEditor.resolvePluginResource(e.manifest.id,e.manifest.ver,"editor/question-ctrl.js");ecEditor.getService(ServiceConstants.POPUP_SERVICE).loadNgModules(t,e)},loadHtml:function(e,t){var o=this;ecEditor.getService(ServiceConstants.POPUP_SERVICE).open({template:"QuestionUnitBrowser",controller:"QuestionCreationFormController",controllerAs:"$ctrl",resolve:{instance:function(){return o},questionData:function(){return t}},width:900,showClose:!1,closeByEscape:!1,closeByDocument:!1,className:"qc-ngdialog-custome"})}}))